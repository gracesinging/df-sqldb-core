...
/**
 * Criteria represents a query criteria, such as conditions, ordering by, limit/offset.
 *
...
class Criteria
{
    ...
    public $limit = -1;
    /**
     * @var integer zero-based offset from where the records are to be returned. If less than 0, it means starting from
     *      the beginning.
     */
    public $offset = -1;
    /**
     ...
     * This property is effective only when the criteria is passed as a parameter to the following methods of
     * CActiveRecord:
     * <ul>
     ...
     * <li>Multiple scopes with parameters:
     * $criteria->scopes=array('scopeName1'=>array($params1),'scopeName2'=>array($params2));</li>
     * <li>Multiple scopes with the same name:
     * array(array('scopeName'=>array($params1)),array('scopeName'=>array($params2)));</li>
     * </ul>
     * @since 1.1.7
     */
    public $scopes;
     ...
     *
     * @return Criteria the criteria object itself
     */
     ...
     * @param string  $keyword  the search keyword. This interpretation of the keyword is affected by the next
     *                          parameter.
     * @param boolean $escape   whether the keyword should be escaped if it contains characters % or _.
     *                          When this parameter is true (default), the special characters % (matches 0 or more
     *                          characters) and _ (matches a single character) will be escaped, and the keyword will be
     *                          surrounded with a % character on both ends. When this parameter is false, the keyword
     *                          will be directly used for matching without any change.
     ...
     *                              intelligent comparison will be conducted. If the value is an array, the comparison
     *                              is done by exact match of any of the value in the array. If the string or the array
     *                              is empty, the existing search condition will not be modified.
     * @param boolean $partialMatch whether the value should consider partial text match (using LIKE and NOT LIKE
     ...
     *                              and _ (matches a single character) will be escaped, and the value will be
     *                              surrounded with a % character on both ends. When this parameter is false, the value
     *                              will be directly used for matching without any change.
     *
     * @return Criteria the criteria object itself
     * @since 1.1.1
     ...
     *
     * @param mixed          $criteria the criteria to be merged with. Either an array or Criteria.
    ...
    public function mergeWith($criteria, $operator = 'AND')
    {
        ...
        if ($this->select !== $criteria->select) {
            if ($this->select === '*') {
                ...
                $select1 =
                    is_string($this->select) ? preg_split('/\s*,\s*/', trim($this->select), -1, PREG_SPLIT_NO_EMPTY)
                        : $this->select;
                $select2 =
                    is_string($criteria->select) ? preg_split('/\s*,\s*/', trim($criteria->select), -1,
            ...
            foreach ((array)$criteria->with as $k => $v) {
                if (is_integer($k)) {
                    ...
                    foreach (array('joinType', 'on') as $opt) {
                        ...
                        if (isset($v[$opt])) {
                            $excludes[$opt] =
                                ($opt === 'on' && isset($excludes[$opt]) && $v[$opt] !== $excludes[$opt]) ?
