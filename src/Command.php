...
class Command
{
     ...
     * @param integer $dataType      SQL data type of the parameter. If null, the type is determined by the PHP type of
     *                               the value.
     ...
     * @param integer $dataType SQL data type of the parameter. If null, the type is determined by the PHP type of the
     *                          value.
     *
     * @return Command the current command being executed
     ...
     *                      you cannot bind parameters or values using {@link bindParam} or {@link bindValue}, and vice
     *                      versa. Please also note that all values are treated as strings in this case, if you need
     *                      them to be handled as their real data types, you have to use {@link bindParam} or {@link
     *                      bindValue} instead.
     *
     ...
     * @param array   $params           input parameters (name=>value) for the SQL execution. This is an alternative
     *                                  to {@link bindParam} and {@link bindValue}. If you have multiple input
     *                                  parameters, passing them in this way can improve the performance. Note that if
     *                                  you pass parameters in this way, you cannot bind parameters or values using
     *                                  {@link bindParam} or {@link bindValue}, and vice versa. Please also note that
     *                                  all values are treated as strings in this case, if you need them to be handled
     *                                  as their real data types, you have to use {@link bindParam} or {@link
     *                                  bindValue} instead.
     *
     ...
     *                      bindValue} instead.
     *
     * @return mixed the value of the first column in the first row of the query result. False is returned if there is
     *               no value.
     * @throws \Exception execution failed
     */
     ...
     * @param array  $params input parameters (name=>value) for the SQL execution. This is an alternative
     *                       to {@link bindParam} and {@link bindValue}. If you have multiple input parameters, passing
     *                       them in this way can improve the performance. Note that if you pass parameters in this
     *                       way,
     *                       you cannot bind parameters or values using {@link bindParam} or {@link bindValue}, and
     *                       vice versa. Please also note that all values are treated as strings in this case, if you
     *                       need them to be handled as their real data types, you have to use {@link bindParam} or
     *                       {@link bindValue} instead.
     *
     ...
     *
     * @throws \Exception if "from" key is not present in given query parameter
     ...
     *                        array('id', 'name')). Columns can contain table prefixes (e.g. "tbl_user.id") and/or
     *                        column aliases (e.g. "tbl_user.id AS user_id"). The method will automatically quote the
     *                        column names unless a column contains some parenthesis
     *                        (which means the column contains a DB expression).
     * @param string $option  additional option that should be appended to the 'SELECT' keyword. For example,
     *                        in MySQL, the option 'SQL_CALC_FOUND_ROWS' can be used. This parameter is supported since
     *                        version 1.1.8.
     *
     ...
     *                      Table names can contain schema prefixes (e.g. 'public.tbl_user') and/or table aliases (e.g.
     *                      'tbl_user u'). The method will automatically quote the table names unless it contains some
     *                      parenthesis
     ...
     * <li><code>or</code>: similar as the <code>and</code> operator except that the operands are concatenated using
     * OR.</li>
     ...
     * <li><code>not in</code>: similar as the <code>in</code> operator except that IN is replaced with NOT IN in the
     * generated condition.</li>
     * <li><code>like</code>: operand 1 should be a column or DB expression, and operand 2 be a string or an array
     * representing the values that the column or DB expression should be like. For example, array('like', 'name',
     * '%tester%') will generate "name LIKE '%tester%'". When the value range is given as an array, multiple LIKE
     * predicates will be generated and concatenated using AND. For example, array('like', 'name', array('%test%',
     * '%sample%')) will generate
     * "name LIKE '%test%' AND name LIKE '%sample%'".
     * The method will properly quote the column name and escape values in the range.</li>
     * <li><code>not like</code>: similar as the <code>like</code> operator except that LIKE is replaced with NOT LIKE
     * in the generated condition.</li>
     * <li><code>or like</code>: similar as the <code>like</code> operator except that OR is used to concatenated the
     * LIKE predicates.</li>
     * <li><code>or not like</code>: similar as the <code>not like</code> operator except that OR is used to
     * concatenated the NOT LIKE predicates.</li>
     * </ul>
     *
     ...
     *                           Table name can contain schema prefix (e.g. 'public.tbl_user') and/or table alias (e.g.
     *                           'tbl_user u'). The method will automatically quote the table name unless it contains
     *                           some parenthesis
     *                           (which means the table is given as a sub-query or DB expression).
     ...
     * @param string $table the table to be joined.
     *                      Table name can contain schema prefix (e.g. 'public.tbl_user') and/or table alias (e.g.
     *                      'tbl_user u'). The method will automatically quote the table name unless it contains some
     *                      parenthesis
     *                      (which means the table is given as a sub-query or DB expression).
     *
     * @return Command the command object itself
     ...
     *                       Columns can be specified in either a string (e.g. "id, name") or an array (e.g.
     *                       array('id', 'name')). The method will automatically quote the column names unless a column
     ...
     *                       Columns can be specified in either a string (e.g. "id ASC, name DESC") or an array (e.g.
     *                       array('id ASC', 'name DESC')). The method will automatically quote the column names unless
     *                       a column contains some parenthesis
     *                       (which means the column contains a DB expression).
     *
     ...
     *                           Do not use column names as parameter names here. They are reserved for
     ...
     * @param string $type   the new column type. The {@link getColumnType} method will be invoked to convert abstract
     *                       column type (if any) into the physical one. Anything that is not recognized as abstract
     *                       type will be kept in the generated SQL. For example, 'string' will be turned into
     *                       'varchar(255)', while 'string not null' will become 'varchar(255) not null'.
     *
    ...
    {
        return $this->setText($this->getConnection()
            ->getSchema()
     ...
     *                        separate them by commas. The column names will be properly quoted by the method.
    ...
    {
        return $this->setText($this->getConnection()->getSchema()->createIndex($name, $table, $column, $unique))
            ->execute();
    }
     ...
     *
     * @param string $name    the name of the primary key constraint to be created. The name will be properly quoted by
     *                        the method.
     * @param string $table   the table who will be inheriting the primary key. The name will be properly quoted by the
     *                        method.
     * @param string $columns the column/s where the primary key will be effected. The name will be properly quoted by
     *                        the method.
     *
     * @return integer number of rows affected by the execution.
     * @since 1.1.13
     ...
     *
     * @param string $name  the name of the primary key constraint to be dropped. The name will be properly quoted by
     *                      the method.
